    ...
    for line in document:
        ...
        while i < len(line):
            ...
            if ch.isalpha() or ch.isdigit():
                token = ch
                i += 1
                while i < len(line) and (line[i].isalpha() or line[i].isdigit() or (line[i] == "'" and line[i+1].isalpha()) or (line[i] == "-" and line[i+1].isalpha())): #om bokstav, om nummer eller om ' eller - följt av bokstav
                    token += line[i]
                    i += 1
                words.append(token)
            else:
...
def countWord(words):
    ...
    nyLista = []
    vardeLista = []
    slutLista = {} #skapar en dictionary
    while i < len(words):
    ...
    while i < len(nyLista): #lägger ihop ord och värde listan i en gemensam dictionary
        slutLista[nyLista[i]] = vardeLista[i] #ord [i] får värdet av värdelista[i]
        i += 1 
    sorteradLista = sorted(slutLista.items(), key=lambda x:x[1], reverse=True)
